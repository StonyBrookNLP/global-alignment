...
from model.baselines.zoo import CodaPrompt
...
class SelfAttention(nn.Module):
    ...
    def self_att(self, x, mask, prompt):
        B, T, C = x.size()
        ...
        if prompt is not None:
            pk, pv = prompt
            pk = pk.reshape(B, -1, self.n_head, C // self.n_head).permute(0, 2, 1, 3)
            pv = pv.reshape(B, -1, self.n_head, C // self.n_head).permute(0, 2, 1, 3)
            k = torch.cat((pk,k), dim=2)
            v = torch.cat((pv,v), dim=2)
            mask = torch.cat((torch.ones((mask.size(0), pk.size(2)), device=mask.device, dtype=mask.dtype), mask), dim=1)
    ...
    def forward(self, x, mask, prompt): 
        y = self.self_att(x, mask, prompt)
...
class Transformer(nn.Module):
    def __init__(self, config, class_head=True, mlm_head = False):
        ...
        #self.task_component = 50
        self.n_component = 200#self.task_component * config.n_tasks
        self.prompt_length = 40
        self.ortho_mu = 0.1
        prompt_param = [self.n_component, self.prompt_length, self.ortho_mu]
        self.coda_prompt = CodaPrompt(config.n_embd, config.n_tasks, prompt_param)
        self.prompt_loss = 0
        self.method = 'coda'
    ...
    def forward(self, x, mask, segment):
        self.prompt_loss = torch.zeros((1,), requires_grad=True).cuda()
        #Embedding layer
        ...
        with torch.no_grad():
            for layer in self.blocks:
                x = layer(x, mask, prompt=None)
            q = x[:,0,:]
        
        for i, layer in enumerate(self.blocks):
            p_list, loss, x = self.coda_prompt.forward(q, i, x, train=self.training)
            self.prompt_loss += loss
            x = layer(x, mask, prompt=p_list)
        return x
    ...
    def class_loss(self, class_predict, target):
        ...
        loss = class_loss + self.prompt_loss.sum()
        return loss
