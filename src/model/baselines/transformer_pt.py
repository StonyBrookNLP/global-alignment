...
class Transformer(nn.Module):
    def __init__(self, config, class_head=True, mlm_head = False):
        ...
        self.n_prompt = 100
        self.prompt = nn.Parameter(torch.FloatTensor(self.n_prompt, config.n_embd).normal_(0, 1))
    ...
    def forward(self, x, mask, segment):
        ...
        prompt = self.prompt.clone().unsqueeze(0).repeat(emb.size(0), 1, 1)

        emb = torch.cat((prompt, emb), dim = 1)
        mask_prompts = torch.ones(mask.size(0), self.n_prompt).to(mask.device)
