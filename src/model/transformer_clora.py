...
class SelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Lora parameters
        r = 8
        self.query_neigh_down = nn.Linear(config.n_embd, r, bias = False)
        self.query_neigh_up = nn.Linear(r, config.n_embd, bias = False)
        self.value_neigh_down = nn.Linear(config.n_embd, r, bias = False)
        self.value_neigh_up = nn.Linear(r, config.n_embd, bias = False)
        self.scale = config.scale #0.1
        self.init_lora()

    def init_lora(self):
        self.query_neigh_up.weight.data.zero_()
        self.value_neigh_up.weight.data.zero_()
        nn.init.kaiming_uniform_(self.query_neigh_down.weight.data, a=math.sqrt(5))
        nn.init.kaiming_uniform_(self.value_neigh_down.weight.data, a=math.sqrt(5))
    
    def self_att(self, x, mask):
        B, T, C = x.size()
        # calculate query, key, values for all heads in batch and move head forward to be the batch dim
        k = self.key(x)
        q = self.query(x) + self.scale * self.query_neigh_up(self.query_neigh_down(x))
        v = self.value(x) + self.scale * self.value_neigh_up(self.value_neigh_down(x))
